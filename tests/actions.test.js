...
describe('Basic actions test', () => {
    it('updates numeric value', async () => {
        ...
        const Bear = () => {
            ...
            return _jsx("div", { children: value.toString() });
        };
    ...
    it('updates array value', async () => {
        ...
        const fishCollections = Array.from({ length: faker.datatype.number({ min: 5, max: 10 }) }, () => randID());
        const bearSliver = createSliver({
            initialState: {
                fishCollections,
            },
            actions: {
                addFish(state, fishId) {
        ...
        const Bear = () => {
            ...
            return _jsx("div", { children: JSON.stringify(value) });
        };
        const bear1 = render(_jsx(Bear, {}));
        const element = await bear1.findByText(JSON.stringify(fishCollections.concat(newFish)));
        expect(renderCount).toBe(2);
        expect(element.innerHTML).not.toEqual(JSON.stringify(fishCollections));
    });
    it('updates array element value', async () => {
        const randID = () => faker.datatype.string();
        const fishCollections = Array.from({ length: faker.datatype.number({ min: 5, max: 10 }) }, () => ({
            id: randID(),
        ...
        const bearSliver = createSliver({
            ...
            actions: {
                updateWeight(state, fishId, value) {
        ...
        expect(renderCount).toBe(2);
        expect(element.innerHTML).not.toEqual(JSON.stringify(fishCollections));
        expect(JSON.stringify(expected)).not.toEqual(JSON.stringify(fishCollections));
    });
});
